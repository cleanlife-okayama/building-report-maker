(function () {
  ...
  function buildReportSectionAiPrompt(sectionKey) {
    ...
    if (sectionKey === "concern") {
      return (
        commonStart +
        "お客様から聞き取った不安・相談・希望を、入力された意味の範囲で分かりやすく整えてください。入力内容が短い場合も、不安、希望、原因、将来の心配、確認したい内容、補修の必要性などを推測で補わず、担当者が入力した意味の範囲だけで、やさしく自然な文章にしてください。入力されていない「雨水の入り込みが心配」「劣化が進まないか心配」「補修範囲を確認したい」などを、一般論として追加しないでください。自然に長くすることよりも、入力内容への忠実さを最優先してください。\n\n" +
        "【現在の入力内容】\n" +
