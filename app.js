(function () {
  ...
  const AI_PHOTO_IMPORTANT_GUIDANCE =
    ...
    "写真で見えている状態は「見られます」「確認できます」のように明確に伝えて構いません。ただし、写真だけでは分からない原因、内部状態、将来必ず起こる被害は断定せず、「可能性があります」「確認が必要です」「おすすめします」などの表現を使ってください。\n\n";
  const AI_PHOTO_PROMPT =
    ...
    "一般のお客様にも伝わる、やさしく自然な説明文にしてください。写真で見える状態と、追加確認が必要なことを分けて書いてください。写真だけで断定できない原因、内部状態、将来必ず起こる被害は断定せず、「可能性があります」「確認が必要です」「おすすめします」などの表現を使ってください。\n\n" +
  ...
  function buildFindingAiConsultationPrompt(finding) {
    ...
    return (
      ...
      "【欄ごとの考え方】\n" +
      "【確認した内容】は、担当者が入力した事実として分かりやすく伝えてください。\n" +
      "【考えられること・注意点】は、不安をあおらず、可能性や注意点として整理してください。\n" +
      "【対応の考え方】は、必要な対応やおすすめする方向性を弱めすぎず、プロの見立てとして自然に伝えてください。例：上から塗装するだけでなく、サビや浮いた塗膜を除去したうえで、必要な補修とサビ止めを行うことをおすすめします。\n\n" +
      "【現在のカード入力】\n" +
      `確認項目名：${area}\n` +
      `状態：${condition}\n` +
      `対応の目安：${priority}\n` +
  ...
  function buildProposalPrimaryAiConsultationPrompt() {
    ...
    return (
      ...
      AI_WRITING_GUIDANCE +
      "\n\n" +
      "施工上必要な内容は明確に伝えてください。入力済みの施工内容については、「必要です」「大切です」「おすすめします」のように分かりやすく書いて構いません。ただし、効果保証、完全に直る、必ず防げる、他社比較、自社へ強く誘導する表現は避けてください。\n\n" +
