(function () {
  ...
  const AI_WRITING_GUIDANCE =
    ...
    "入力内容にない劣化原因、工事内容、効果、危険性を断定して追加しないでください。不明な点に限り「可能性があります」「確認が必要です」「検討できます」などの表現にしてください。\n" +
    "文字数は目安として扱い、不自然に削らず、短さよりも分かりやすく伝わることを優先してください。";
  const AI_BUILDING_EXPERT_CORE_GUIDANCE =
    "【建築専門補佐の共通ルール】\n" +
    "1. 担当者が入力・選択した事実、見立て、施工方針を最優先してください。\n" +
    "2. 確認できた劣化や不具合は明確に伝え、原因、内部状態、将来被害、工法、材料は、入力がなければ断定しないでください。\n" +
    "3. 分からない内容は「可能性があります」「詳しい確認が必要です」など、確認できた事実と分けてください。\n" +
  ...
  const AI_OUTPUT_SAFETY_GUIDANCE =
    ...
    "・入力内容にない劣化原因、写真だけでは分からない内部状態、将来必ず起こる被害、効果保証、価格、法的判断、保証、他社比較は断定しないでください。\n" +
    "・写真や入力内容から分からないことは断定しないでください。\n" +
    "・断定できない内容に限り、「可能性があります」「確認が必要です」「検討できます」などの表現を使ってください。確認できる事実や入力済みの見立てまで弱めすぎないでください。\n" +
  ...
  const PDF_TEXT_FIELD_RULES = {
    ...
    proposalClosing: { label: "最後にお伝えしたいこと", maxLength: 800, warningRatio: 0.85 },
  ...
  const fieldPlaceholders = {
    ...
    最後にお伝えしたいこと:
      "今回の確認内容をもとに、工事の優先順位や今後の確認事項、お客様に安心して判断いただくための補足を記入してください。\n500文字前後を目安に、しっかり書く場合は600〜700文字程度、長くても800文字以内がおすすめです。\n必要な対応と今後見ておきたい点を整理し、相談しやすさや確認の姿勢が伝わる内容にしてください。",
  };
  ...
  const defaultState = {
    ...
    proposal: {
      ...
      closing: "今回確認した内容をもとに、必要な対応と今後見ておきたい点を整理しました。気になる点があれば、分かる範囲で一つずつご説明いたします。",
    },
    processDiagram: {
      enabled: false,
  ...
  function hasFindingAiPhotoSource(photo) {
    return (
      !isBlank(aiConsultationValue(photo?.condition)) ||
      !isBlank(photo?.finding) ||
      !isBlank(photo?.memo) ||
      shouldShowPhotoRecommendation(photo)
    );
  }

  function hasFindingAiSource(finding) {
  ...
  function buildFindingRelatedPhotoAiReference(finding) {
    ...
    return (
      "【関連写真の参考情報】\n" +
      "以下は、この確認項目に紐づく写真カードの入力内容です。確認項目全体の文章を整理するための参考情報として扱い、回答見出しは増やさないでください。写真タイトルと撮影箇所は場所や文脈の補助として使い、それだけを状態判断の根拠にはしないでください。\n" +
      lines.join("\n\n")
    );
  }
  ...
  function buildFindingAiConsultationPrompt(finding) {
    ...
    return (
      ...
      "この相談文の目的は、新しく診断することではありません。確認項目と関連写真に担当者が入力・選択した情報を整理し、一般のお客様にも伝わる自然な文章案を作ることです。\n\n" +
      AI_BUILDING_EXPERT_CORE_GUIDANCE +
      "\n\n" +
      "次の3項目を整えてください。\n【確認した内容】\n【考えられること・注意点】\n【対応の考え方】\n\n" +
      "回答は必ず上記3つの見出しだけにしてください。見出し名は変更せず、感想、解説、総評、追加の改善提案は書かないでください。各欄が未入力でも、確認項目と関連写真の入力情報から分かる範囲で文章案を作ってください。\n\n" +
      "【欄ごとの考え方】\n" +
      "【確認した内容】は、確認項目と関連写真に入力された事実を統合する欄です。複数写真に共通して確認できる状態、写真ごとの場所や状態の違い、一部または複数箇所に見られる状態、確認できた範囲を、確認項目全体として読みやすくまとめてください。写真ごとの文章をそのまま順番に並べず、補修方法、施工方法、将来の予測は混ぜすぎないでください。\n" +
      "【考えられること・注意点】は、確認した事実と選択済みの現在の状態をもとに、お客様が写真だけでは気づきにくい注意点を整理する欄です。そのままにした場合の心配、経過確認や詳しい確認が必要な理由、劣化が広がった場合に考えられる影響を、入力内容から分かる範囲で伝えてください。原因、内部状態、雨漏りの発生、将来必ず起こる被害、危険性は断定しないでください。\n" +
      "【対応の考え方】は、選択済みの「今後の対応」を判断の軸として、何をどのように確認・清掃・調整・補修・改修・交換するかを整理する欄です。作業内容の羅列にせず、なぜその対応を考えるのか、何を確認してから判断するのか、現在の状態と対応がどうつながるのかを分かりやすく伝えてください。選択済みの「今後の対応」と矛盾する文章を作らないでください。「現時点で対応不要」や「定期的な経過確認」で強い補修・交換を勧めず、「詳しい確認を行う」では原因や内部状態を決めつけずに追加確認後の判断としてください。「早めの対応が必要」「直ちに対応が必要」でも不安をあおらず、確認済みの事実と対応理由を明確にしてください。\n\n" +
      "【複数の関連写真をまとめるときの考え方】\n" +
      "同じ内容を写真枚数分繰り返さず、共通する状態はまとめてください。場所や状態に違いがある場合は、その違いが伝わるようにしてください。一部の写真だけに見られる状態を全体へ広げて断定せず、写真数だけで劣化範囲や危険度を判断しないでください。関連写真に入力されていない原因、内部状態、工事内容は作り足さないでください。写真タイトルと撮影箇所は文脈として使い、状態の根拠にはしないでください。\n\n" +
      "【現在のカード入力】\n" +
      `確認項目名：${area}\n` +
      `現在の状態：${condition}\n` +
      `今後の対応：${priority}\n` +
      ...
      "断定できない内容に限り「可能性があります」「確認が必要です」「検討できます」などの表現を使ってください。確認できる事実や、入力済みの担当者の見立てまで弱めすぎないでください。\n" +
      "「確認した内容」に対応方針を入れすぎず、「考えられること・注意点」に原因や将来被害を断定して入れず、「対応の考え方」に具体的な確認・補修・塗装・清掃・交換の方向性を整理してください。\n" +
  ...
  async function copyFindingAiConsultationPrompt(findingId) {
    const finding = state.findings.find((item) => item.id === findingId);
    if (!finding) return;
    if (!hasFindingAiSource(finding)) {
      window.alert(
        "確認項目または関連写真に、確認できた状態などを入力してください。\n" +
          "写真タイトルや撮影箇所だけでは、確認結果の文章を作成できません。",
      );
      return;
    }
  ...
  function buildProposalAiConsultationPrompt() {
    ...
    return (
      ...
      "【工事中に確認が必要なこと】260字以内\n" +
      "【今後注意しておきたい点】220字以内\n" +
      "【最後にお伝えしたいこと】500〜700文字程度を目安に、長くても800文字以内\n" +
      "各欄の文字数目安内で、必要な説明は省略しすぎず、お客様が納得して判断できる文章にしてください。短くまとめることよりも、分かりやすく伝わることを優先してください。無理に文字数を増やさず、理由・背景・不安への配慮・対応する意味が自然に伝わる文章を優先してください。短い例を入れた方が伝わりやすい場合は、自然な範囲で入れてください。ただし、入力内容にない原因・工事内容・効果・危険性を断定して追加しないでください。\n\n" +
      "【最後にお伝えしたいことの考え方】\n" +
      "短い締め文だけで終わらせず、冒頭に現地確認の機会をいただいたことへのお礼を入れてください。今回確認した内容をもとに、現在の状態と今後の対応目安を整理したこと、すぐ対応した方がよい部分と状態を見ながら考えてよい部分を分けて説明する姿勢、足場設置後や工事中に追加確認が必要な点が分かった場合はその都度説明しながら進めること、気になる点を一つずつ確認しながら進めることを自然に含めてください。毎回すべてを機械的に入れず、入力内容、確認項目、写真情報、施工方針に合うものを選んでください。2〜4段落程度に分け、同じ内容の繰り返しは避けてください。姿勢を自己評価として説明するのではなく、必要な対応と今後見ておきたい点を整理して伝える文章にしてください。断定しすぎる言い方、自社へ任せるよう強く促す言い方、他社との比較、完全・絶対・必ず・100%のような保証的な表現は避けてください。\n\n" +
      "【重要な注意事項】\n" +
  ...
  function buildProposalPrimaryAiConsultationPrompt() {
    ...
    return (
      ...
      AI_BUILDING_EXPERT_CORE_GUIDANCE +
      "\n\n" +
      "施工方針は、工事内容をただ並べる欄ではありません。今回確認した状態を受けて、なぜその工事が必要なのか、どのように進めるのか、お客様が判断しやすい形で整理してください。\n" +
      "施工上必要な内容は明確に伝えてください。入力済みの施工内容については、「必要です」「大切です」「行います」「確認します」「検討します」のように分かりやすく書いて構いません。ただし、効果保証、完全に直る、必ず防げる、他社比較、自社へ強く誘導する表現は避けてください。\n\n" +
      "【欄ごとの考え方】\n" +
      "【ご提案内容】は、工事の大きなテーマを短くまとめる欄です。工事項目を細かく並べすぎず、例：雨水まわりとサビ対策を重視した屋根保護／下地処理を重視した塗替え、のように方向性が伝わる表現にしてください。\n" +
      ...
      "【注意事項】\n" +
      "入力内容にない劣化原因、内部状態、工事内容、効果、危険性を勝手に追加しないでください。\n" +
      "完全に直る、必ず防げる、絶対に安心、今すぐ工事しないと危険、他社より優れている、のような保証的・比較的・不安をあおる表現は避けてください。\n" +
      "断定できない内容に限り「可能性があります」「確認が必要です」「検討します」などの表現を使ってください。確認済みの状態や、施工上必要な作業まで弱めすぎないでください。\n" +
      "強い売り込みではなく、現場の状態を踏まえて必要な対応を整理する文章にしてください。"
    );
  }
  ...
  function proposalSupplementAiConfig(fieldKey) {
    const configs = {
      additional: {
        ...
        guidance:
          "この欄は、追加提案を強くすすめる欄ではなく、今回の確認内容や施工方針とあわせて確認・検討しておくと安心な対策を整理する欄です。現場状況とつながる理由を短く入れ、「確認します」「検討します」「整えます」など報告書本文として自然な語尾にしてください。「おすすめします」は必要な場合だけ使い、売り込みに見える表現は避けてください。",
      },
      cautions: {
        label: "工事中に確認が必要なこと",
        ...
        guidance:
          "この欄は、工事中に確認する可能性がある点を事前に共有し、あとから「聞いていない」と感じにくくするための欄です。下地、漏水、劣化範囲、追加補修の可能性など、現場で確認しながら進める内容を整理してください。「工事中に確認します」「状態に応じてご説明します」「必要な場合はその都度確認します」のように少しはっきり伝えて構いません。ただし、追加工事や追加費用が必ず発生するような表現にはしないでください。",
      },
      watchPoint: {
        label: "今後注意しておきたい点",
        ...
        guidance:
          "この欄は、工事後や今後の維持管理で見ておきたい点を整理する欄です。不安をあおらず、定期的な確認、変化があった場合の早めの相談、雨水まわりやサビなどの経過確認につながる自然な文章にしてください。将来必ず起こる被害として断定せず、「確認します」「見ていきます」「状態に応じて対応します」のような落ち着いた表現にしてください。",
      },
      closing: {
        label: "最後にお伝えしたいこと",
        ...
        guidance:
          "この欄は、報告書の最後に、今回の確認内容、施工方針、注意点、今後の確認事項を踏まえて、お客様が判断しやすいように整理する説明欄です。短い締め文ではなく、担当者の考え方、優先順位の分け方、確認しながら進める姿勢、相談しやすさが自然に伝わる文章として整えてください。回答本文は、必ず現地確認の機会をいただいたことへのお礼から始めてください。例として「このたびは、現地確認の機会をいただきありがとうございました。」のように書き、入力内容から自然に分かる場合は、屋根、波板、鉄部など確認した箇所を含めても構いません。続けて、今回確認した内容をもとに現在の状態と今後の対応目安を整理したこと、すぐ対応した方がよい部分と状態を見ながら考えてよい部分を分けて説明する姿勢、足場設置後や工事中に再確認する場合があること、追加で分かったことがあればその都度説明しながら進めることを自然に伝えてください。最後は、気になる点や分かりにくい点を一つずつ確認しながら進められることが伝わる、やさしい締めにしてください。ただし、これらを毎回すべて機械的に入れず、入力内容、確認項目、関連写真、施工方針に合うものを自然に選んでください。姿勢を自己評価として説明するのではなく、必要な対応と今後見ておきたい点を整理して伝える文章にしてください。「適しています」「分かりやすいです」のようなAIが評価している表現や、自社へ任せるよう強く促す表現、他社との比較、完全・絶対・必ず・100%のような保証的な表現は避けてください。500〜700文字程度のまとまった文章にし、文字数を埋めるための繰り返しは避け、2〜4段落程度で読みやすくしてください。",
      },
    };
  ...
  function buildProposalSupplementAiConsultationPrompt(fieldKey) {
    ...
    return (
      ...
      `【${config.label}】\n${config.ruleKey === "proposalClosing" ? "このたびは、現地確認の機会をいただきありがとうございました。\n本文\n\n" : "本文\n\n"}` +
      "【文字数の目安】\n" +
      `【${config.label}】${limitGuide}\n` +
      "PDF上で読みやすく収まるよう、文字数目安内で自然な文章にしてください。入力内容にある理由、背景、お客様の不安、現場状況はできるだけ活かしてください。\n\n" +
      "【注意事項】\n" +
      "写真や入力内容だけで断定できないことは断定しないでください。\n" +
      "断定できない内容に限り「可能性があります」「確認が必要です」「検討します」などの表現を使ってください。確認済みの状態や、入力済みの見立てまで弱めすぎないでください。\n" +
      "「おすすめします」は毎回使わず、必要な場合だけ自然に使ってください。基本は「確認します」「進めます」「検討します」「整えます」「状態に応じて対応します」のような報告書本文の語尾にしてください。\n" +
      "「適しています」「分かりやすいです」のようにAIが文章を評価している表現は避けてください。\n" +
  ...
  function buildReportSectionAiPrompt(sectionKey) {
    ...
    if (sectionKey === "concern") {
      return (
        commonStart +
        "お客様から聞き取った内容をもとに、何を心配されているのか、何を確認したいのかが伝わる文章へ整えてください。入力内容が短い場合も、不安、希望、原因、将来の心配、確認したい内容、補修の必要性などを推測で補わず、担当者が入力した意味の範囲だけで、やさしく自然な文章にしてください。入力されていない「雨水の入り込みが心配」「劣化が進まないか心配」「補修範囲を確認したい」などを、一般論として追加しないでください。自然に長くすることよりも、入力内容への忠実さを最優先してください。\n\n" +
        "【現在の入力内容】\n" +
        ...
        "文字数目安内で、入力された不安や希望を省略せず、入力内容の意味の範囲だけで自然な文章にしてください。短い入力を自然に長くすることよりも、入力内容への忠実さを優先してください。入力にない確認希望、対応希望、原因、将来の心配、補修の必要性は追加しないでください。「確認してほしい」「相談したい」「補修が必要か知りたい」などの希望は、入力に明記されている場合だけ反映してください。\n\n" +
